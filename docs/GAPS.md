...
| **G-50** | ZK Contingent Payments | [FULL_STACK_BITCOIN_RESEARCH.md#50](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#50) | 🛡️ **Fail-closed platform boundary** in `zkcp.ts`; versioned intent binding, bounded retention/pagination, immutable evidence lifecycle, pre-release safe-clock validation, deterministic key derivation, durable lookup-first/idempotent-release contract validation, and restart/ambiguity reconciliation are enforced, while ZK verifier, payment observer, and the external durable key-release backend remain pending |
...
- **G-50 (ZKCP)**: 🛡️ **Fail-closed boundary; versioned intent binding, bounded retention/pagination, immutable evidence revalidation, safe pre-release clocks, deterministic bounded release keys, and durable lookup-first/idempotent-release reconciliation are implemented, but verifier/payment/key-release backends remain pending. Exactly-once depends on the external durable backend contract, not BFF memory**.
