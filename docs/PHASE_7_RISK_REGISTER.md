...
| **R-7-06** | Verifier scaffolding can be mistaken for cryptographic settlement authorization | Security / Correctness | High | Critical | **CRITICAL** | @platform-lead | Issue #1187 quarantines simulation and length-only/contradictory success, requires adapter-owned proof/payment bindings, canonical string-bound detached attestation evidence with pre-parse limits, versioned ZKCP intent terms and retention/list quotas, bounded pre-hash inputs and adapter errors, canonical versioned BitVM signatures, versioned BitVM2 hard retention/reservations with active-operation preservation, bounded BitVM3 recursion metadata and terminal retention plus finite-window identity tombstones, per-intent/per-proof FIFO/CAS replay protection, and rejection of caller-only payment evidence. Production ZKCP key release is completely removed: no adapter, registry, obligation execution, decryption-key output, or finalized success. Any future irreversible coordinator requires independent authentication, server binding, Gateway/Core atomic claim-or-get, and a durable registry; dependency injection alone is never sufficient. | Before Phase 7 launch |
...
- [x] **[R-7-06]** Add fail-closed verifier/payment contracts, pre-parse canonical attestation payload limits, versioned BitVM2 retention/reservations, bounded BitVM3 terminal retention with finite-window tombstones, async lifecycle/replay protection, ZKCP retention/pagination, atomic signer reservations, bounded identifier logging, and contamination guards (Issue #1187); production key release is hard-disabled, and any future independently authenticated server-bound Gateway/Core atomic claim-or-get coordinator plus durable registry remains a separate gate.
