...
- **Integrity:** digest-only handover `integrity.digest`, artifact `digest` values, context `allowlist_digest`, and each context entry's `provenance_digest`. Authentication is an envelope-only concern; a handover authentication field is rejected.
...
Use `packageContext()` to build a snapshot from caller-supplied values and a versioned/digested allowlist derived from a validated #1162 discovery result, `resolveContextSnapshot()` to re-check freshness at resume time, and `mergeContextSnapshots()` to combine snapshots deterministically. `redactSensitiveFields()` is available when preparing non-sensitive task data. These helpers are pure and do not read the filesystem, environment, network, or provider transcript.
