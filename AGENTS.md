...
**What was done**:
- Added current local-development and deployment-surface guidance with exact direct/Compose ports, placeholder/stub boundaries, `make auth` limits, and fail-closed external handoffs.
- Repaired active local links and stable research anchors; added a deterministic local Markdown link validator, focused tests, root scripts, and pinned documentation-only CI.
- Corrected active AGENTS facts, added operational freshness metadata, and preserved the #1167 authority deferral.
**Key discoveries**:
- `make auth` does not generate `ADMIN_DASHBOARD_API_KEY` or `SERVICE_KEY_*`; Compose Gateway/UI defaults are placeholders and optional protocol profiles are RPC stubs.
- The repository now has 19 tracked workflow definitions, and the Conxian organization has 16 repositories.
**Files touched**:
- `README.md`, `CONTRIBUTING.md`, `AGENTS.md`, active documentation/OpenSpec links, `docs/LOCAL_DEVELOPMENT.md`, `docs/DEPLOYMENT.md`, `scripts/check-markdown-links*.mjs`, `package.json`, and `.github/workflows/docs-validation.yml`.
**Gaps identified**:
- Protocol/Nexus/Gateway/Platform authority remains deferred to #1167; production deployment selection and external service implementations remain out of scope.
**Gotchas**:
- Historical/archive paths and prior session entries are intentionally excluded from repair and link gating; the local pnpm configuration emits a non-fatal missing `NPM_TOKEN` interpolation warning.
