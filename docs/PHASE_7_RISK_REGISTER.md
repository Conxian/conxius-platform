...
| **R-7-06** | Verifier scaffolding can be mistaken for cryptographic settlement authorization | Security / Correctness | High | Critical | **CRITICAL** | @platform-lead | Issue #1187 quarantines simulation and length-only/contradictory success, requires adapter-owned authoritative backend bindings, canonical string-bound detached attestation evidence with pre-parse limits, versioned ZKCP intent terms and retention/list quotas, bounded pre-hash inputs and adapter errors, canonical versioned BitVM signatures, versioned BitVM2 hard retention/reservations with active-operation preservation, bounded BitVM3 recursion metadata and terminal retention plus finite-window identity tombstones, per-intent/per-proof FIFO/CAS replay protection, safe pre-release ZKCP clocks, stable encrypted-data obligation identity, pinned durable registry namespace, lookup-first atomic claim/conflict semantics, bounded flat key-release evidence, immutable evidence, bounded identifier logs, attested aggregation, and rejection of caller-only payment evidence. Durable Gateway/Core identity retention, the external key-release exactly-once/lookup contract, and accepted production backends remain separate launch gates; BFF memory alone is not an exactly-once guarantee. | Before Phase 7 launch |
...
- [x] **[R-7-06]** Add fail-closed verifier/payment contracts, pre-parse canonical attestation payload limits, versioned BitVM2 retention/reservations, bounded BitVM3 terminal retention with finite-window tombstones, async lifecycle/replay protection, ZKCP retention/pagination plus pre-release clock and stable-obligation registry/lookup/claim/conflict hardening, atomic signer reservations, bounded identifier logging, and contamination guards (Issue #1187); durable identity storage, the external key-release exactly-once contract, and backend enablement remain separate gates.
