...
Otherwise it only prints fallback messages; it does not deploy Gateway to GCP
or UI to Render. Treat deployment execution and evidence as owned by the
external operator/repository, not guaranteed by this target.
