...
| **G-50** | ZK Contingent Payments | [FULL_STACK_BITCOIN_RESEARCH.md#50](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#50) | 🛡️ **Fail-closed platform boundary** in `zkcp.ts`; versioned intent binding, bounded retention/pagination, atomic immutable evidence lifecycle, pre-release safe-clock validation, and one-shot key-release latching are enforced, while ZK verifier, payment observer, and key-release backends remain pending |
...
- **G-50 (ZKCP)**: 🛡️ **Fail-closed boundary; versioned intent binding, bounded retention/pagination, immutable evidence revalidation, safe pre-release clocks, and one-shot release latching are implemented, but verifier/payment/key-release backends remain pending**.
