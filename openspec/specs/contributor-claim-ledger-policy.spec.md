...
This specification defines the normative policy for contributor recognition claims in the Conxius platform.
...
### 7.3 Append-only mutation model

Implementations MUST NOT overwrite prior state transitions.

Every transition, dispute, cap deferment, and revocation MUST be represented by an appended event record with:
...
4. Governance ratifies activation and snapshot coordinates (`H_activate`, `T_activate`).
...
1. Freeze eligible recognized CU at snapshot (`H_activate`, `T_activate`).
