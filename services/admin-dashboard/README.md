# Conxius Admin Dashboard

This service is the internal backend/frontend orchestration layer for the Conxian platform. It provides real-time telemetry from the Unified Gateway Engine and allows for management of institutional secrets.

## ⚠️ SECURITY WARNING

**This dashboard is for internal, local, or high-privilege administrative use only.**

- **Secret Exposure**: The `/settings` page allows viewing and modifying institutional secrets (NPM, GCP, GitHub, etc.).
- **Access Control**: Currently intended for internal, local-only use. DO NOT expose this service to the public internet without implementing a robust authentication layer (e.g., VPN, mTLS, or RBAC).
- **Environment Hygiene**: Use `.env.admin` for local development ONLY. This file is intentionally ignored by Git to prevent accidental leakage of institutional credentials.

## 🔐 Security Hardening

To maintain the security of this platform:
1. **Local Only**: By default, run the dashboard on `localhost`.
2. **Template Usage**: Use `.env.admin.example` as a base for your local configuration.
3. **Audit Logs**: Future versions will include automated logging of all secret modifications to the Nexus Glass Node for auditability.
4. **Secret Storage**: The dashboard currently saves secrets to `.env.admin` for local persistence. In production environments, it is recommended to use a secure vault (e.g., Vault, AWS Secrets Manager, GCP Secret Manager).

## Features

- **Infrastructure Pulse**: Real-time monitoring of Gateway health, Engine version, and request throughput.
- **Nexus "Glass Node" State**: Visibility into Merkle roots and synchronization status with Stacks L1.
- **Sovereign Services Tracking**: Status monitoring for Stacks (L2), Bisq (P2P), RGB, BitVM, and Lightning Network.
- **Secret Management**: Interface for provisioning institutional secrets and BOS wallet mappings (accessible via `/settings`).

## Tech Stack

- **Framework**: Next.js 15
- **Language**: TypeScript
- **State Management**: React Hooks (`useState`, `useEffect`)

## Getting Started

### Local Development

1.  Navigate to the service directory:
    ```bash
    cd services/admin-dashboard
    ```
2.  Install dependencies:
    ```bash
    pnpm install
    ```
3.  Configure secrets:
    ```bash
    cp .env.admin.example .env.admin
    # Edit .env.admin with actual values (DO NOT COMMIT)
    ```
4.  Run the development server:
    ```bash
    pnpm dev
    ```
    The dashboard will be available at [http://localhost:3001](http://localhost:3001).

### Generated Files

- `next-env.d.ts` is generated by Next.js on first run and is intentionally not tracked.
- `.env.admin` is used for local persistence of settings and is ignored by Git.

## Architecture

- `src/app/`: Next.js App Router for UI components and API routes.
- `src/app/api/secrets/`: API routes for reading/updating operational configuration and secrets.
- `src/app/settings/`: Enterprise Management Console settings for high-privilege configuration.

## RBAC & Security

Access to this dashboard is currently intended for internal use. Future iterations will include Role-Based Access Control (RBAC) integrated with `conxian-access.clar`.

### Governance & Standards

- Follows [SECURITY.md](../../SECURITY.md) for reporting.
- Adheres to [CONTRIBUTING.md](../../CONTRIBUTING.md) monorepo guidelines.
- Complies with [CODEOWNERS](../../CODEOWNERS) service ownership.
