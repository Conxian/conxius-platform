...
def repair_api():
    ...
    # Standardize field names for PartnerLeadCreateInput
    content = content.replace("partner_name", "name")
    content = content.replace("contact_name", "email") # Placeholder to match repair script

    # More robust fix: replace the validate method entirely
    validate_method = """    fn validate(self) -> Result<PartnerLeadCreateInput, Vec<String>> {
        Ok(PartnerLeadCreateInput {
            name: self.name.unwrap_or_default(),
            email: self.email.unwrap_or_default(),
            organization: self.company_name.unwrap_or_default(),
            initial_note: self.notes,
        })
    }"""
    ...
    if len(matches) > 1:
        ...
        attr_start = content.rfind("#[get", 0, second_start)
        block_end = content.find("}", second_start) + 1
        content = content[:attr_start] + content[block_end:]
