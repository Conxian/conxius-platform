...
| **G-10** | MuSig2 Aggregation (Active Scaffolding & Unit Verified) | [FULL_STACK_BITCOIN_RESEARCH.md#31](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#31) | `services/admin-dashboard/src/lib/support/musig2.ts` |
...
| **G-53** | Lightning Async Payments | [FULL_STACK_BITCOIN_RESEARCH.md#53](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#53) | 🟢 **Implemented & Unit Verified**. HTLC hold invoices & bounded async payment state machine in `services/admin-dashboard/src/lib/support/asyncPayments.ts`. |
| **G-54** | OP_VAULT (BIP-345) | [FULL_STACK_BITCOIN_RESEARCH.md#54](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#54) | 🟢 **Active Scaffolding & Unit Verified**. BIP-345 covenant state machine, timelock maturity, emergency recovery, and capacity bounds in `services/admin-dashboard/src/lib/support/opVault.ts` verified by `opVault.test.ts`. |
...
| **G-64** | BitVMX Fail-Closed Execution & Bounded State Machine | [FULL_STACK_BITCOIN_RESEARCH.md#44](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#44) | 🛡️ **Fail-closed boundary & Unit Verified**. Strict program/input validation, state transition invariants, bounded challenge depth, memory limits, and tombstoning in `services/admin-dashboard/src/lib/support/bitvmx.ts`. |
| **G-65** | Conxian Unified API Token Management & Developer Self-Service | [CONXIAN_API_TOKEN_SPEC.md](./architecture/CONXIAN_API_TOKEN_SPEC.md) | 🟢 **Implemented & Unit Verified**. Prefixed token generator (`cx_live_` / `cx_test_`), SHA-256 hashed storage, scope delegation, and management API endpoints in `services/admin-dashboard`. |
| **G-66** | Unified Client System Installer & Provisioning Engine | [CLIENT_ONBOARDING_AND_UNIFIED_INSTALLER_SPEC_2026.md](./architecture/CLIENT_ONBOARDING_AND_UNIFIED_INSTALLER_SPEC_2026.md) | 🟢 **Implemented & Unit Verified**. Declarative client setup engine, environment validator, and license/M2M token binder in `services/admin-dashboard/src/lib/support/installer.ts`. |
