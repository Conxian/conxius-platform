...
2.  **ISO 20022 & Institutional Bridge (CON-76, CON-63)**:
    *   [DONE] Implement OData v4 parsers and pacs.008 wrapper in the Gateway.
...
4.  **Phase 6 Sovereign Primitives (Alignment)**:
    ...
    *   [DONE] Implement Nexus Glass Node state synchronization (`/api/v1/nexus/state`).
...
1.  **Core API Client Extension**:
    *   [DONE] Update .services/conxian-ui/src/lib/core-api.ts. with Phase 6 methods: `getAiAllocation`, `getUbiIdentity`, `getNexusState`.
2.  **System Telemetry Integration**:
    *   [DONE] Refactor UI .SystemStatus. component to fetch live data from the Gateway's unified telemetry instead of contract mocks.
3.  **Component "Vibe-Verification"**:
    *   [DONE] Audit all UI components (AiAllocationCard, NexusSyncStatus, UbiIdentityCard) for adherence to the "Sovereign Earthy" theme.
