...
The implementation is concentrated in `scripts/agent-coordination.ts`, with contract tests in `scripts/agent-coordination.test.ts`, the machine-readable schema in `schemas/agent-swarm.schema.json`, focused root scripts and CI coverage, and links from `docs/AGENT_ONBOARDING.md` and `docs/SESSION_CONTINUITY.md`. The canonical normative source is `openspec/specs/swarm-coordination-v1.spec.md`.
