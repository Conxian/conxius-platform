...
| **G-50** | ZK Contingent Payments | [FULL_STACK_BITCOIN_RESEARCH.md#50](./architecture/FULL_STACK_BITCOIN_RESEARCH.md#50) | 🛡️ **Fail-closed platform boundary** in `zkcp.ts`; stable encrypted-data obligation identity, pinned registry namespace, lookup-by-obligation, atomic claim/conflict semantics, bounded flat canonical evidence, versioned intent binding, bounded retention/pagination, immutable evidence lifecycle, pre-release safe-clock validation, and restart/ambiguity reconciliation are enforced, while ZK verifier, payment observer, and the external durable key-release backend remain pending |
...
- **G-50 (ZKCP)**: 🛡️ **Fail-closed boundary; stable encrypted-data obligation identity, pinned durable registry namespace, lookup-first atomic claim/conflict semantics, bounded flat canonical evidence, versioned intent binding, bounded retention/pagination, immutable evidence revalidation, safe pre-release clocks, deterministic binding keys, and restart/ambiguity reconciliation are implemented, but verifier/payment/key-release backends remain pending. Exactly-once depends on the external durable coordinator contract, not BFF memory**.
