...
The Markdown handover remains the human-readable compatibility format. For automated resumption, create a `handover.v1` document with `createHandover(input, graph, { allowlist, discovery })` and validate it with `validateHandover(handover, { graph, allowlist, discovery })` or `assessHandoverResumability(handover, { graph, allowlist, discovery, now })`. When both formats are present, the machine-readable record is authoritative for lifecycle state, task status, digests, conflicts, risks, blockers, and resume actions; Markdown may explain those fields but must not override them.
...
- **Integrity and provenance:** digest-only handover `integrity.digest`, artifact `digest` values, context `allowlist_digest`, and each context entry's `provenance_digest`. Every handover boundary also revalidates the derived #1162 `ContextAllowlist` against its source `DiscoveryResult` and checks every embedded context source; a locally recomputed entry/snapshot/handover digest does not authorize an unallowlisted path. Authentication is an envelope-only concern; a handover authentication field is rejected.
...
Use `packageContext()` to build a snapshot from caller-supplied values and a versioned/digested allowlist derived from a validated #1162 discovery result, `resolveContextSnapshot(snapshot, now, { allowlist, discovery })` to re-check freshness with provenance at a handover boundary, and `mergeContextSnapshots()` to combine snapshots deterministically. `redactSensitiveFields()` is available when preparing non-sensitive task data. These helpers are pure and do not read the filesystem, environment, network, or provider transcript.
